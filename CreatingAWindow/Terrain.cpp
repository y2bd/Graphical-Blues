...
bool Terrain::GenerateHeightmap(int numIterations)
{
    ...
    for (int k = 0; k < numIterations; k++)
    {
        ...
        float xCoord = ((float)rand() / RAND_MAX) * width;
        float zCoord = ((float)rand() / RAND_MAX) * height;
        float sign = rand() >(RAND_MAX >> 1) ? 1 : -1;
        float raiseHeight;
        if (++iteration < iterThreshold)
            raiseHeight = maxHeight + ((float)iteration / iterThreshold) * (minHeight - maxHeight);
        for (int z = floor(zCoord - radius); z < ceil(zCoord + radius); z++)
        {
            for (int x = floor(xCoord - radius); x < ceil(xCoord + radius); x++)
            {
                ...
                if (dist < radius)
                {
                    ...
                    heightMap[z][x] += sqrt(radius * radius - dist * dist) * height;
                }
            }
        }
    }
    ...
    {
        if (((unsigned char *)heightMap)[i] < min) { min = ((unsigned char *)heightMap)[i]; }
        if (((unsigned char *)heightMap)[i] > max) { max = ((unsigned char *)heightMap)[i]; }
    }
    for (int i = 0; i < width * height; i++)
    {
        ((unsigned char *)heightMap)[i] = (((unsigned char *)heightMap)[i] - min) / (max - min);
    }
    ...
    for (unsigned int j = 0; j < height; ++j)
    {
        ...
        {
            unsigned int index = (j * width) + i;
            float heightValue = heightMap[j][i];
    ...
    std::cout << "Terrain has been generated!" << std::endl;
    //delete[] heightMap;
...
void Terrain::GenerateNormals()
{
    ...
    for (unsigned int i = 0; i < m_NormalBuffer.size(); ++i)
    {
        ...
        float fTexture0Contribution = glm::dot(m_NormalBuffer[i], UP) - 0.1f;
